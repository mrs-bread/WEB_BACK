...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    // Значения: из БД для авторизованного или из куки
    $values = [];
    if (!empty($_SESSION['login']) && !empty($_SESSION['uid'])) {
        // загрузим данные заявки
...
} else setVal('gender', $_POST['gender']);
...
// Работа: новая заявка или правка
if (!empty($_SESSION['login']) && !empty($_SESSION['uid'])) {
    // правим existing
    ...
    // языки: удалим старые и вставим новые
    ...
    // новая заявка
    $db->beginTransaction();
    $appStmt = $db->prepare("INSERT INTO application (fio,phone,email,birth_date,gender,bio) VALUES (?,?,?,?,?,?)");
    $appStmt->execute([
        trim($_POST['fio']), trim($_POST['phone']), trim($_POST['email']),
        $_POST['birth_date'], $_POST['gender'], trim($_POST['bio'])
    ]);
    $appId = $db->lastInsertId();
    $linkStmt = $db->prepare("INSERT INTO application_language (application_id,language_id) VALUES (?,?)");
    $findLang = $db->prepare("SELECT id FROM programming_languages WHERE language_name=?");
    foreach($_POST['languages'] as $l){
        $findLang->execute([$l]);
        if ($lid = $findLang->fetchColumn()){
    ...
    // сохраняем в user_auth
    $auth = $db->prepare("INSERT INTO user_auth (application_id,login,password_hash) VALUES (?,?,?)");
    $auth->execute([$appId,$login,$hash]);
    $db->commit();
    // отдадим пользователю
    setcookie('login',$login);
    setcookie('pass',$pass);
}

// ставим признак сохранения и редирект
setcookie('save','1');
header('Location: index.php');
exit();