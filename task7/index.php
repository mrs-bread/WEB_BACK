    ...
    if (!empty($_COOKIE['save'])) {
        ...
        if (!empty($_COOKIE['login']) && !empty($_COOKIE['pass'])) {
            $messages[] = sprintf(
                ...
                htmlspecialchars($_COOKIE['login'], ENT_QUOTES|ENT_SUBSTITUTE, 'UTF-8'),
                htmlspecialchars($_COOKIE['pass'], ENT_QUOTES|ENT_SUBSTITUTE, 'UTF-8')
            );
...
// === Обработка POST: проверка CSRF ===
if (empty($_POST['csrf_token']) || !hash_equals($_SESSION['csrf_token'], $_POST['csrf_token'])) {
    http_response_code(403);
    die('Ошибка проверки безопасности (CSRF).');
}
